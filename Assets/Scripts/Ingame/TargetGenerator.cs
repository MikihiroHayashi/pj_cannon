    ...
    public void GenerateTargetsForStage(int stageIndex)
    {
        ...
        for (int i = 0; i < stageConfig.targetCount; i++)
        {
            ...
            if (placementSuccessful)
            {
                ...
                // ターゲットのスコア値を設定（プレハブの値を優先）
                // プレハブの元のscore値が0より大きい場合は、その値を使用
                // そうでない場合は、ステージ設定のスコア範囲からランダムに設定
                if (targetComponent.scoreValue <= 0)
                {
                    targetComponent.scoreValue = Random.Range((int)stageConfig.scoreRange.x, (int)stageConfig.scoreRange.y + 1);
                    Debug.Log($"ターゲットのスコアをランダム設定: {targetComponent.scoreValue}");
                }
                else
                {
                    Debug.Log($"ターゲットのプレハブスコア値を使用: {targetComponent.scoreValue}");
                }
