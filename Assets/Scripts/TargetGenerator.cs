...
// ターゲット生成クラス - ScriptableObject対応版（修正完了）
public class TargetGenerator : MonoBehaviour
{
    ...
    public void InitializeCurrentStage()
    {
        ...
        if (config != null)
        {
            Debug.Log($"ステージ設定確認: {config.stageName}, ターゲット数={config.targetCount}");
        }
        else
        {
    ...
    public void ClearTargets()
    {
        ...
        // 1. まずシーン内のすべてのターゲットを検索して削除
        GameObject[] allTargetsInScene = GameObject.FindGameObjectsWithTag("Target");
        int sceneTargetsCount = allTargetsInScene.Length;
        
        if (sceneTargetsCount > 0)
        {
            Debug.Log($"シーン内のターゲットを削除します: {sceneTargetsCount}個");
        ...
        // 2. 生成リスト内のターゲットを削除（念のため）
        int listTargetsCount = generatedTargets.Count;
        if (listTargetsCount > 0)
        {
            Debug.Log($"生成リスト内のターゲットを削除します: {listTargetsCount}個");
        ...
        // 4. 二重確認
        allTargetsInScene = GameObject.FindGameObjectsWithTag("Target");
        if (allTargetsInScene.Length > 0)
        {
            Debug.LogWarning($"クリア後もまだターゲットが残っています: {allTargetsInScene.Length}個。強制的にクリアします。");
        ...
        if (initializedStages.ContainsKey(currentStage))
        {
            ...
            Debug.Log($"ステージ {currentStage} を未初期化状態にリセットしました");
        }
    ...
    public void GenerateTargetsForStage(int stageIndex)
    {
        ...
        if (successfulPlacements != targetCountToGenerate || generatedTargets.Count != targetCountToGenerate)
        {
            ...
            if (duplicates > 0)
            {
                Debug.LogError($"generatedTargetsリスト内に {duplicates} 個の重複が見つかりました");
            }
            
            // 多すぎる場合は余分なターゲットを削除
            if (generatedTargets.Count > targetCountToGenerate)
            {
                Debug.LogWarning($"余分なターゲットを削除します: {generatedTargets.Count - targetCountToGenerate}個");
                
                int extraCount = generatedTargets.Count - targetCountToGenerate;
                for (int i = 0; i < extraCount; i++)
                {
                    int lastIndex = generatedTargets.Count - 1;
                    GameObject extraTarget = generatedTargets[lastIndex];
                    ...
                    if (extraTarget != null)
                    {
                        if (Application.isPlaying)
                            ...
                            DestroyImmediate(extraTarget);
                    }
                    
                    generatedTargets.RemoveAt(lastIndex);
                }
                
                Debug.Log($"調整後のターゲット数: {generatedTargets.Count}");
            }
        }
    }
